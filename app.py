                        ...
                        with cols[j]:
                            with st.container(border=True):
                                ...
                                if '마지막 방문일' in item and pd.notnull(item['마지막 방문일']):
                                    # Timestamp를 datetime으로 변환하여 계산
                                    last_date = item['마지막 방문일'].to_pydatetime()
                                    today = datetime.now()
                                    diff = (today - last_date).days
                                    
                                    if diff >= 30:
                                        st.error(f"🚨 마지막 방문 후 {diff}일 경과")
                                    elif diff >= 20:
                                        st.warning(f"🟡 마지막 방문 후 {diff}일")
                                    else:
                                        st.success(f"✅ 방문 후 {diff}일 (안정)")
                                else:
                                    st.info("ℹ️ 방문 기록 없음")
